...
var today_timestamp = 0 - 1485907200;
var one_month_from_now_timestamp = 0 - 1491004800;
...
function load_events_this_month(first, last) {
  ref.orderByChild('timestamp').startAt(one_month_from_now_timestamp).endAt(today_timestamp).once('value')
...
function load_events_by_page(first, last) {
...
function load_data() {
  ...
  load_events_this_month(first, last);
  load_buttons();
}
